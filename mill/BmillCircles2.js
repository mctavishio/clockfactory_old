...
let drawp = {
	...
	vline: p => { return {x1:p.cx,x2:p.cx,y1:0,y2:1,"stroke-width":p.sw,"stroke-dasharray":0.8*p.sd,"stroke-opacity":1.0,stroke:p.color } },
...
let colors = ["warmlightwhite","red","warmlightwhite", "warmblack"].map(c=>pigments[c]);
 //let colors = ["#fdfdf3","#fdfdf3","#191918","#fdfdf3"];
...
let Bfilm = {
	...
	const istweens = [[Math.floor(nticks*0.28),1],[1,0]].flatMap(wx=>{
...
//| [...new Array(nlayers-1).keys()].map( z => z+1).forEach( z => { 
	[0,1].forEach( j => {
		//=let colors = ["warmlightwhite","red","yellow","warmlightwhite"].map(c=>pigments[c]);
		let colors = ["warmlightwhite","red","warmlightwhite"].map(c=>pigments[c]);
		let l = tools.randominteger(0,nlayers+1);
		//|let l = tools.randominteger(0,nlayers);
		let color = colors[tools.randominteger(0,colors.length)];
